...
export function getBucketName(): string {
  ...
  if (!bucket) {
    console.warn(
      "NEXT_PUBLIC_SUPABASE_BUCKET is not defined, using default bucket: item_images"
    );
    return "item_images";
  }
  return bucket;
...
export async function createBucket(bucketName = getBucketName()) {
  try {
    ...
    if (!bucketExists) {
      // Create the bucket
      const { data, error } = await adminSupabase.storage.createBucket(
        bucketName,
        {
          public: true,
...
export async function updatePhotoCount(
  itemId: string,
  table = "items"
): Promise<boolean> {
  try {
